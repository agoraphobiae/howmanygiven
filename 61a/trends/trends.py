...
def extract_words(text):
    ...
    #     text[i] = "".join(i for i in text[i] if i in string.ascii_letters + " ")
    for c in text:
...
def analyze_tweet_sentiment(tweet):
    ...
    for word in tweet_words(tweet):
        val = make_sentiment(get_word_sentiment(word))
        if has_sentiment(val):
            total += sentiment_value(val)
            count += 1
    if count != 0: #if total != 0 was the old one
...
def find_center(polygons):
    ...
    for polygon in polygons:
        ...
        centerx += centroid[0]*centroid[2]
        centery += centroid[1]*centroid[2]
        areasum += centroid[2]

    return (centerx/areasum, centery/areasum)
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        ...
        # tweets_by_state[state] = tweets_by_state.get(state,list()).append(tweet)
        # print(tweets_by_state[state])
...
def most_talkative_state(term):
    ...
    "*** YOUR CODE HERE ***"
    tweets_states = group_tweets_by_state(tweets)
    maxtweets = 0
    maxtweetsstate = ""

    for state in tweets_states:
        tweets_states[state] = len(tweets_states[state])
        if(tweets_states[state] >= maxtweets):
            maxtweets = tweets_states[state]
            maxtweetsstate = state

    #print(tweets_states)

    return maxtweetsstate
...
def average_sentiments(tweets_by_state):
    ...
    "*** YOUR CODE HERE ***"
    # divisor should be how many have values
    # states with tweets with no sentiments, should be left out
    for state in tweets_by_state:
        ...
        for tweet in tweets_by_state[state]:
            ...
            if(has_sentiment(s)):
                ...
                statecount += 1
        if statecount != 0:
            stateavg = stateavg / statecount # len(tweets_by_state[state])
            if stateavg != 0:
                # if no sentiment don't add the state to the dictionary
                averaged_state_sentiments[state] = stateavg

    return averaged_state_sentiments

    # THIS HERE KIDDOS HAS NO STATECOUNT
    # for state in tweets_by_state:
    #     stateavg = 0
    #     for tweet in tweets_by_state[state]:
    #         s = analyze_tweet_sentiment(tweet)
...
def group_tweets_by_hour(tweets):
    ...
    "*** YOUR CODE HERE ***"
    for tweet in tweets:
        tweethour = tweet_time(tweet).hour
        if tweethour in tweets_by_hour:
            tweets_by_hour[tweethour].append(tweet)
        else:
            tweets_by_hour[tweethour] = [tweet]

    #for hour in tweets_by_hour:
    #    print(len(tweets_by_hour[hour]))
